...
constexpr size_t N=TerrainPager::Samples;
...
int main(int argc,char** argv) {
    ...
    for(int mode=0;mode<5;++mode) {
        ...
        for(auto& tile:tiles) {
            ...
            en+=std::chrono::duration<double>(Clock::now()-start).count();total+=n;commit+=TerrainPager::Committed(n);
        ...
        printf("mode=%d tiles=%zu ratio=%.6f projected_gib=%.3f commit_gib=%.3f encode_us=%.1f decode_us=%.1f exact=1\n",
