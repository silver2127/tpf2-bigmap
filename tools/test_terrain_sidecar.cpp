...
int main() {
    ...
    { FILE* f = nullptr; OpenFile(&f, path, "rb"); fseek(f, 0, SEEK_END); long sz = ftell(f); fclose(f);
      std::vector<uint8_t> buf(sz); OpenFile(&f, path, "rb"); assert(fread(buf.data(), 1, sz, f) == size_t(sz)); fclose(f);
      const char* tp = "test_sidecar_trunc.bin"; OpenFile(&f, tp, "wb"); fwrite(buf.data(), 1, sz - 100, f); fclose(f);
    ...
    { FILE* f = nullptr; OpenFile(&f, path, "rb"); fseek(f, 0, SEEK_END); long sz = ftell(f); fclose(f);
      ...
      const char* cp = "test_sidecar_corrupt.bin"; OpenFile(&f, cp, "wb"); fwrite(buf.data(), 1, sz, f); fclose(f);
    ...
    // caller falls back to compute for it) while every other tile restores.
    {
        ...
        const char* cp = "test_sidecar_1bad.bin"; OpenFile(&f, cp, "wb"); fwrite(buf.data(), 1, sz, f); fclose(f);
    ...
    // ---- Fingerprint + arm/begin/end + WriteForSave glue. ----
    {
        ...
        { FILE* f = nullptr; OpenFile(&f, sav, "wb"); std::mt19937 r(9); std::vector<uint8_t> b(300000); for (auto& x : b) x = uint8_t(r()); fwrite(b.data(), 1, b.size(), f); fclose(f); }
        ...
        { FILE* f = nullptr; OpenFile(&f, sav, "ab"); uint8_t z = 0; fwrite(&z, 1, 1, f); fclose(f); }
        ArmForLoad(sav);
