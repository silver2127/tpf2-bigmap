...
// Keep CRT portability local to the sidecar; do not emulate Windows globally.
inline int OpenFile(FILE** out, const char* path, const char* mode) {
#ifdef _WIN32
...
inline int CompareExtension(const char* a, const char* b) {
#ifdef _WIN32
    return _stricmp(a, b);
#else
...
// page and miss a profile folder with a non-ASCII name. POSIX paths use
// UTF-8 bytes directly; Windows retains its wide-path implementation.
#ifdef _WIN32
...
#else
inline FILE* OpenFile(const char* utf8, const wchar_t* mode) {
    // Only ASCII CRT mode strings cross this interface, never path text.
    char narrow[8] = {};
    size_t i = 0;
    for (; mode[i]; ++i) {
        if (i + 1 >= sizeof narrow || mode[i] > 127) return nullptr;
        narrow[i] = char(mode[i]);
    }
    return std::fopen(utf8, narrow);
}
inline void RemoveFile(const char* utf8) { std::remove(utf8); }
#endif
...
#else
inline uint64_t FingerprintOf(const char* path) {
    FILE* f = OpenFile(path, L"rb");
    if (!f) return 0;
    FileHeader hdr{};
    bool ok = fread(&hdr, sizeof hdr, 1, f) == 1 && hdr.magic == Magic &&
              hdr.version == Version && hdr.headerHash == HashHeader(hdr);
    fclose(f);
    return ok ? hdr.fingerprint : 0;
}
inline bool FindByFingerprint(uint64_t fingerprint, char* path, size_t cap) {
    if (!fingerprint || !path || !cap || !memchr(path, 0, cap) || !path[0]) return false;
    if (FingerprintOf(path) == fingerprint) return true;
    const char* slash = strrchr(path, '/');
    if (!slash) return false; // Same folder-qualified lookup contract as Windows.
    const std::string dir(path, size_t(slash - path) + 1);
    DIR* d = opendir(dir.c_str());
    if (!d) return false;
    bool found = false;
    while (const dirent* entry = readdir(d)) {
        const size_t n = strlen(entry->d_name);
        if (n < 5 || strcmp(entry->d_name + n - 5, ".terr")) continue;
        const std::string candidate = dir + entry->d_name;
        if (candidate.size() + 1 > cap || FingerprintOf(candidate.c_str()) != fingerprint) continue;
        memcpy(path, candidate.c_str(), candidate.size() + 1);
        found = true;
        break;
    }
    closedir(d);
...
inline void SidecarPath(const char* savPath, char* out, size_t cap) {
    ...
    const char* ext = (n >= 4 && !CompareExtension(savPath + n - 4, ".sav")) ? savPath + n - 4 : savPath + n;
...
#endif
 long BigmapTestSidecarWrite(void* cterrain, uint64_t fp, const char* path, uint64_t* bytesOut) {
...
__declspec(dllexport)
#endif
 long BigmapTestSidecarApply(void* cterrain, uint64_t fp, const char* path) {
